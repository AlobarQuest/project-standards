...
def load(path: Path) -> list[dict]:
    ...
    if not isinstance(entries, list):
        raise ExceptionsError(
            f"{path}: 'exception' must be a list of tables, got {type(entries).__name__}"
        )
    for i, entry in enumerate(entries):
        if not isinstance(entry, dict):
            raise ExceptionsError(
                f"{path}: exception[{i}] must be a table, got {type(entry).__name__}"
            )
...
def local_matches(entry: dict, standard: str, finding_id: str) -> bool:
    return entry["standard"] == standard and fnmatch.fnmatch(finding_id, entry["finding"])
