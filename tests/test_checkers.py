...
from portfolio.checkers import (
    _run,
    check_code,
    check_governance,
    check_infra,
...
from portfolio.matrix import PASS, UNKNOWN, VIOLATION
...
def test_check_security_no_block_findings_is_pass_with_warn_detail(
    monkeypatch, make_repo, tmp_path
):
    repo = make_repo("x")
    monkeypatch.setenv("SECURITY_STANDARDS_REPO", str(tmp_path / "security-standards"))
    finding = {
        "rule_id": "secrets.weak_pattern",
        "severity": "WARN",
        "file": "app.py",
        "line": 5,
        "evidence": "...",
        "remediation": "...",
        "reason": "possible weak pattern",
        "kind": "regex",
    }
...
def test_check_security_missing_summary_key_is_unknown(monkeypatch, make_repo):
    ...
    def fake_run(cmd, cwd=None, env=None, timeout=None):
        return subprocess.CompletedProcess(
            args=[], returncode=0, stdout=json.dumps({"meta": {}}), stderr=""
        )
...
def test_check_security_non_dict_by_severity_is_unknown(monkeypatch, make_repo):
    repo = make_repo("x")
    monkeypatch.setattr(
        checkers, "_run", _run_returning(json.dumps({"summary": {"by_severity": "nope"}}))
    )
    result = check_security(repo)
    assert result.status == UNKNOWN
    assert "unreadable" in result.note
...
def test_check_security_block_finding_null_line_renders_dash(monkeypatch, make_repo):
    repo = make_repo("x")
    finding = {
        "rule_id": "manifest.drift",
        "severity": "BLOCK",
        "file": ".bws-secrets.toml",
        "line": None,
        "evidence": "...",
        "remediation": "...",
...
def test_check_infra_violation_by_name_match_prefers_description(monkeypatch, tmp_path):
    ...
    assert result["myapp-repo"].details == [
        {"id": "572:bd9d2439", "message": "env var drift on myapp"}
    ]
...
def test_check_infra_empty_dir_is_all_unknown_no_report(monkeypatch, tmp_path):
    ...
    result = check_infra({"a": ["x"]}, datetime.now(UTC))
...
def test_check_infra_two_date_files_uses_newer(monkeypatch, tmp_path):
    ...
    old_proposal = {
        "id": "1:aaa",
        "target": {"uuid": "u-1", "name": "old-app"},
        "description": "old drift",
    }
    new_proposal = {
        "id": "2:bbb",
        "target": {"uuid": "u-2", "name": "new-app"},
