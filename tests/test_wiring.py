                    ...
                    "Stop": [
                        {
                            ...
                            "hooks": [
                                {
                                    "type": "command",
                                    "command": "/Users/devon/.claude/hooks/bws-scan-gate.sh",
                                }
                            ],
                        }
                    ]
...
def test_launchagent_plist_existence(tmp_path, machine):
    ...
    assert ok.status == "pass"
    bad = check_required_checks(repo, [{"id": "s", "executor": "launchagent:com.devon.nope"}], True)
    assert _ids(bad) == ["checks.not-wired"]
